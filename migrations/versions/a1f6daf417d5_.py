...
def upgrade():
    ...
    sa.Column('avatar_url', sa.String(length=320), nullable=True),
    sa.Column('banner_url', sa.String(length=320), nullable=True),
    ...
    sa.ForeignKeyConstraint(['editor'], ['user_profiles.id'], ),
    sa.PrimaryKeyConstraint('id')
    )
    op.create_index(op.f('ix_missions_city'), 'missions', ['city'], unique=False)
    op.create_index(op.f('ix_missions_state'), 'missions', ['state'], unique=False)
    op.create_index('name_like', 'missions', ['name'], unique=False, postgres_ops={'name': 'text_pattern_ops'})
    op.create_index('order_date_shared_desc', 'missions', [sa.text('date_shared DESC')], unique=False, postgresql_using='btree', postgres_ops={'date_shared': 'DESC'})
    ...
    sa.Column('mission_id', sa.Integer(), nullable=False),
    sa.Column('goals_completed', sa.PickleType(), nullable=True),
    sa.ForeignKeyConstraint(['mission_id'], ['missions.id'], ),
    sa.ForeignKeyConstraint(['user_id'], ['user_profiles.id'], ),
